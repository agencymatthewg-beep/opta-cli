...
from opta import __version__, models, urls, utils
from opta.analytics import Analytics
from opta.args import get_parser
from opta.coders import Coder
from opta.coders.base_coder import UnknownEditFormat
from opta.commands import Commands, SwitchCoder
from opta.copypaste import ClipboardWatcher
from opta.deprecated import handle_deprecated_model_args
from opta.format_settings import format_settings, scrub_sensitive_info
from opta.history import ChatSummary
from opta.io import InputOutput
from opta.llm import litellm  # noqa: F401; properly init litellm on launch
from opta.models import ModelSettings
from opta.onboarding import offer_openrouter_oauth, select_default_model
from opta.repo import ANY_GIT_ERROR, GitRepo
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run opta in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track opta's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".opta"):
            patterns_to_add.append(".opta*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the opta browser feature",
        ["opta-chat[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Opta-AI/opta/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Opta-AI/opta/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".opta.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading opta model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".opta" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    model_metadata_files += generate_search_path_list(
        ".opta.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Opta only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run opta --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Opta respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve opta?"
            )
    ...
    if args.optaignore:
        ignores.append(args.optaignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from opta.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".opta" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install opta properly?")
