...
from opta import prompts, utils
...
class GitRepo:
    repo = None
    opta_ignore_file = None
    opta_ignore_spec = None
    opta_ignore_ts = 0
    opta_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if opta_ignore_file:
            self.opta_ignore_file = Path(opta_ignore_file)

    def commit(self, fnames=None, context=None, message=None, opta_edits=False, coder=None):
        """
        ...
        Args:
            ...
            opta_edits (bool, optional): Whether the changes were made by Aider. Defaults to False.
        ...
        Aider generated the changes (`opta_edits`).
        ...
        - opta_edits=True: Changes were generated by Aider (LLM).
        - opta_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (opta)".
        - --attribute-committer: Modify Committer name to "User Name (opta)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: opta (<model>) <opta@opta.chat>" trailer to commit message.

        Behavior Summary:

        1. When opta_edits = True (AI Changes):
        ...
        2. When opta_edits = False (User Changes):
        ...
        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(opta),
          Committer=You(opta)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(opta), Committer=You, Trailer added
        - User commit (defaults): opta_edits=False -> Author=You, Committer=You(opta)
        - User commit with explicit no-committer: opta_edits=False,
        ...
        prefix_commit_message = opta_edits and (
        ...
        use_attribute_author = (
            opta_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of opta_edits (based on tests).
        ...
        use_attribute_committer = effective_committer and (
            not (opta_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "opta: " + commit_message
        ...
        committer_name = f"{original_user_name} (opta)"
    ...
    def refresh_opta_ignore(self):
        if not self.opta_ignore_file:
            return

        current_time = time.time()
        if current_time - self.opta_ignore_last_check < 1:
            return

        self.opta_ignore_last_check = current_time

        if not self.opta_ignore_file.is_file():
            return

        mtime = self.opta_ignore_file.stat().st_mtime
        if mtime != self.opta_ignore_ts:
            self.opta_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.opta_ignore_file.read_text().splitlines()
            self.opta_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.opta_ignore_file or not self.opta_ignore_file.is_file():
            return False
        ...
        return self.opta_ignore_spec.match_file(fname)
