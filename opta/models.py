...
MODEL_SETTINGS = []
with importlib.resources.open_text("opta.resources", "model-settings.yml") as f:
...
class ModelInfoManager:
    ...
    def __init__(self):
        self.cache_dir = Path.home() / ".opta" / "caches"
...
class Model(ModelSettings):
    def __init__(
        ...
        self.extra_model_settings = next(
            (ms for ms in MODEL_SETTINGS if ms.name == "opta/extra_params"), None
        )
    ...
    def configure_model_settings(self, model):
        ...
        if (
            ...
            and self.extra_model_settings.name == "opta/extra_params"
        ):
        ...
        if "GITHUB_COPILOT_TOKEN" in os.environ:
            if "extra_headers" not in kwargs:
                kwargs["extra_headers"] = {
                    "Editor-Version": f"opta/{__version__}",
    ...
    def simple_send_with_retries(self, messages):
        ...
        while True:
            try:
                ...
                from opta.reasoning_tags import remove_reasoning_content
