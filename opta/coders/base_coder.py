...
from opta import __version__, models, prompts, urls, utils
...
from opta.io import ConfirmGroup, InputOutput
from opta.linter import Linter
from opta.llm import litellm
from opta.models import RETRY_TIMEOUT
...
from opta.repomap import RepoMap
from opta.run_cmd import run_cmd
from opta.utils import format_content, format_messages, format_tokens, is_image_file
from opta.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_opta_commit_hash = None
    opta_edited_files = None
        ...
        **kwargs,
        ...
        if from_coder:
            ...
            update = dict(
                ...
                opta_commit_hashes=from_coder.opta_commit_hashes,
    ...
    def get_announcements(self):
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .optaignore"
                )
    ...
    def __init__(
        ...
        if opta_commit_hashes:
            self.opta_commit_hashes = opta_commit_hashes
        else:
            self.opta_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches optaignore spec.")
                continue
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.opta_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, opta_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_opta_commit_hash = commit_hash
        self.opta_commit_hashes.add(commit_hash)
        self.last_opta_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each opta commit.")
