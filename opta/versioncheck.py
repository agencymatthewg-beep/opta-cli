...
import opta
from opta import utils
from opta.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = Path.home() / ".opta" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of opta from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of opta from the main branch?",
        ["git+https://github.com/Aider-AI/opta.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of opta from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer opta version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of opta?"
...
"""
    ...
    if success:
        io.tool_output("Re-run opta to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/opta-cli/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = opta.__version__
