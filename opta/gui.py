...
from opta import urls
from opta.coders import Coder
from opta.dump import dump  # noqa: F401
from opta.io import InputOutput
from opta.main import main as cli_main
from opta.scrape import Scraper, has_playwright
...
def search(text=None):
    results = []
    for root, _, files in os.walk("opta"):
        for file in files:
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_opta_commit_hash:
                show_undo = True
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            st.warning(
                "This browser version of opta is experimental. Please share feedback in [GitHub"
    ...
    def do_recommended_actions(self):
        ...
        with st.expander("Recommended actions", expanded=True):
            ...
            with st.popover("Update your `.gitignore` file"):
                st.write("It's best to keep opta's internal files out of your git repo.")
                self.button("Add `.opta*` to `.gitignore`", key=random.random(), help="?")
    ...
    def initialize_state(self):
        ...
        self.state.init("last_opta_commit_hash", self.coder.last_opta_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.opta_edited_files,
            )
            if self.state.last_opta_commit_hash != self.coder.last_opta_commit_hash:
                edit["commit_hash"] = self.coder.last_opta_commit_hash
                edit["commit_message"] = self.coder.last_opta_commit_message
                commits = f"{self.coder.last_opta_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_opta_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_opta_commit_hash = self.coder.last_opta_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_opta_commit_hash != commit_hash
            or self.coder.last_opta_commit_hash != commit_hash
        ):
